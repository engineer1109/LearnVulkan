...
layout (location = 5) in float inDistance;
...
void main() 
{
    ...
    vec4 colorReflect = texture(samplerCubeMap, cR, lodBias);
    
    float reflectPara = 0.2f;

    vec4 colorA = texture(samplerTextureA, inUV.xy, 1.0f);
    vec4 colorB = texture(samplerTextureB, inUV.xy, 1.0f);
    vec4 color = (colorA*0.5+colorB*0.5) * (1.f - reflectPara) + reflectPara * colorReflect;
    color.r = color.r * inColor.r;
    color.g = color.g * inColor.g;
    color.b = color.b * inColor.b;
