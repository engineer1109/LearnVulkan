...
void VulkanTexture2D::loadFromFile(std::string file, VkFormat format,
    ...
    if (useStaging) {
        ...
        uint8_t *data;
        VK_CHECK_RESULT(vkMapMemory(device->logicalDevice, stagingMemory, 0, memReqs.size, 0,
                                    (void **) &data));
...
void VulkanTexture2D::loadFromData(void *imgData,
                                   uint32_t width,
                                   uint32_t height,
                                   uint32_t channels,
                                   VkFormat format,
                                   ...
                                   VkImageLayout imageLayout,
                                   bool forceLinear) {
    this->device = m_context->vulkanDevice;
    this->mipLevels = static_cast<uint32_t>(1);
    this->width = width;
    this->height = height;
    this->channels = channels;
    m_size = width * height * channels * mipLevels * GetPixelSize(format);
    auto copyQueue = m_context->queue;
    ...
    samplerCreateInfo.minFilter = VK_FILTER_LINEAR;
    samplerCreateInfo.mipmapMode = VK_SAMPLER_MIPMAP_MODE_LINEAR;
    samplerCreateInfo.addressModeU = m_addressMode[0];
    samplerCreateInfo.addressModeV = m_addressMode[1];
    samplerCreateInfo.addressModeW = m_addressMode[2];
    samplerCreateInfo.mipLodBias = 0.0f;
    samplerCreateInfo.compareOp = VK_COMPARE_OP_NEVER;
    samplerCreateInfo.minLod = 0.0f;
    // Max level-of-detail should match mip level count
